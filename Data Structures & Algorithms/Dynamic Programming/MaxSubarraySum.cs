...
namespace _PA3
{
	public class MaxSubarraySum
	{
		...
		public static int[] dynamicProgram(int[] A, int length)
		{
			...
			for (int i = 1; i < length; i++)
			{
				...
				{
					globalMax = localMax;
					globalEndIndex = localStartIndex;
					globalEndIndex = i;
				}
			}
			return new int [3] {globalMax, globalStartIndex, globalEndIndex };
		}

		public static int[] divideAndConquer(int[] A, int length)
		{
			return divideAndConquer(A, 0, length - 1);
		}

		private static int[] divideAndConquer(int[] array, int left, int right)
		{
			if (left == right)
			{
		...
		private static int[] cross(int[] array, int left, int right, int mid)
		{
		...
		public static int[] bruteForce(int[] A, int length)
		{
			int globalMax = A[0];
